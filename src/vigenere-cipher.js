      ...
      message.split('').reverse().join('') : message;
		...
		for (let i = 0; i < message.length; i++) {
			(message[i].match(/^[A-Za-z]$/)) ? 
				(encryptStr += String.fromCharCode(getEncode(message[i], keyWordEnc[keyCount++]))) : 
      ...
      encryptedMessage.split('').reverse().join('') : encryptedMessage;
		...
		for (let i = 0; i < encryptedMessage.length; i++) {
			(encryptedMessage[i].match(/^[A-Za-z]$/)) ? 
				(decryptStr += String.fromCharCode(getDecode(encryptedMessage[i], keyWordDec[keyCount++])) ) : 
